...
character_y_pos = screen_height - character_height - \
    stage_height  # on the bottom of screen
...
while running:
    ...
    weapons = [[w[0], w[1] - weapon_speed]
               for w in weapons]  # 무기 위치를 위로 쭉 올리는것
    ...
    for ball_index, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):  # 변수 선언해주고
                ...
                if ball_img_idx < 3 : # 가장 작은 공이 아니라면 나눠주기
                    ...
                    #나눠진 공 정보
                    small_ball_rect = ball_images[ball_img_idx + 1].get_rect() #작아진 공
                    ...
                    balls.append({
                        ...
                        "to_x": -3,  # 공의 x축 이동 방향 -이면 왼쪽
                        "to_y": -6,
                        "init_spd_y": ball_speed_y[ball_img_idx + 1]  # y최초속도
                        })
                    # 오른쪽으로 튕겨나가는 작은 공 
                    balls.append({
                        "pos_x": ball_pos_x + (ball_width / 2) - (small_ball_width/2),  # 공의 x 좌표
            ...
            continue #안쪽 for문 조건이 맞지 않으면 continue 바깥 for문 계속 진행
        break #안쪽 for문에서 break를 만나면 여기로 진입가능 2중 for문을 한번 더 빠져나간다. 
